...
class PID_controller {
	...
	float max_iterm;
	// ema filter value for proportional controller input
	float ema_filter_p;
	...
	// error between setpoint (sp) and process variable (pv)
	float error, error_old;
	// error for derivative controller
	float error_d, error_d_old;
	...
	// constructor
	PID_controller(float K_p_new, float K_i_new, float K_d_new, float min_mv_tol_new, float min_mv_new, float max_mv_new, float max_iterm_new, float ema_filter_p_new = 1, float ema_filter_d_new = 1);
